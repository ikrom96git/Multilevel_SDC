...
class collocation_problem(object):
    ...
    def get_collocation_fine_model(self, U):
        ...
        for ii in range(self.coll.num_nodes):
            ...
            for jj in range(self.coll.num_nodes):
                ...
                pos_equation+=-self.prob.dt*self.coll.Q[ii+1, jj+1]*self.prob.u0[1]-self.prob.dt**2*self.coll.QQ[ii+1, jj+1]*self.get_rhs_fine(u)[1]
                vel_equation+=-self.prob.dt*self.coll.Q[ii+1, jj+1]*self.get_rhs_fine(u)[1]
    ...
    def get_collocation_coarse_model(self, U, X0, V0):
        ...
        for ii in range(self.coll.num_nodes):
            ...
            for jj in range(self.coll.num_nodes):
                u=np.array([U[jj], U[self.coll.num_nodes+jj]])
                pos_equation+=-self.prob.dt*self.coll.Q[ii+1, jj+1]*self.prob.u0[1]-self.prob.dt**2*self.coll.QQ[ii+1, jj+1]*self.get_rhs_coarse(u)[1]
                vel_equation+=-self.prob.dt*self.coll.Q[ii+1, jj+1]*self.get_rhs_coarse(u)[1]
            equation=np.concatenate([pos_equation, vel_equation])
    ...
    def compute_integral_coarse(self, X, V):
        ...
        pos_int = (
            X - (self.prob.dt * self.coll.Q @ V0
            + self.prob.dt**2 * self.coll.QQ @ build_f)
        )
        return pos_int, vel_int
