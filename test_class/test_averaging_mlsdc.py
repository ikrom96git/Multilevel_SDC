...
def test_mlsdc_zeros_averaging():
    ...
    (
        ...
        sweeper_params_sdc,
        problem_class_sdc,
    ) = get_sdc_default_params(Force="Fast_time")
    model_sdc = sdc_class(
        problem_params_sdc,
        collocation_params_sdc,
        sweeper_params_sdc,
...
def test_mlsdc_first_averaging():
    ...
    model_averaged_first_order_mlsdc = Mlsdc_class(
        problem_params_reduced, collocation_params, sweeper_params, problem_class_first
    )
    model_mlsdc.get_mlsdc_iter_solution()
    ...
    Residual_averaged_first = (
        model_averaged_first_order_mlsdc.sdc_fine_level.get_residual
    )
    Kiter = np.arange(1, sweeper_params["Kiter"] + 1, 1)
