...
def test_mlsdc_arg_min_sweep():
    ...
    label_set = [
        "MLSDC ",
        "$0^{th}$ order arg min model",
        "$0^{th}$ order averaged model",
    ]
    residual_set = [
        np.array(Residual_mlsdc)[:, 0],
        np.array(Residual_reduced)[:, 0],
...
def test_mlsdc_arg_min_first_order():
    ...
    label_set = [
        "MLSDC ",
        "$0^{th}$ order arg min model",
        "$1^{st}$ order arg min model",
    ]
    residual_set = [
        np.array(Residual_mlsdc)[:, 0],
        np.array(Residual_zeros_order)[:, 0],
        np.array(Residual_first_order)[:, 0],
    ]
    plot_residual(Kiter, residual_set, Title, label_set)
...
def mlsdc_simple_test():
    ...
    model_mlsdc_simple = Mlsdc_class(
        problem_params, collocation_params, sweeper_params, problem_class_first_order
    )
    model_mlsdc.get_mlsdc_iter_solution()
