...
def test_fast_time_SDC():
    eps = 0.001
    problem_fast_time_params, time_fast = get_harmonic_oscillator_fast_time_params(
        Fast_time=True
    )
    *_, collocation_params, sweeper_params, problem_class = get_sdc_default_params()
    ...
    model_sdc = sdc_class(
        problem_fast_time_params,
        collocation_params,
        sweeper_params,
        problem_class_fast_time,
    )
...
def test_residual_fast_time():
    K = 20
    problem__fast_time_params, *_ = get_harmonic_oscillator_fast_time_params(
        Fast_time=True
    )
    *_, collocation_params, sweeper_params, problem_class = get_sdc_default_params()
    problem_class_fast_time = HarmonicOscillator_fast_time
...
def test_fast_time_first_order_model():
    ...
    position = model_first_order.asyp_expansion(
        solution_fast[0, :], solution_first[0, :], eps=0.001
    )
    Title = "Solution of Fast time"
...
def test_fast_time_first_order_sdc():
    ...
    *_, collocation_params, sweeper_params, problem_class = get_sdc_default_params()
    problem_class_fast_time = HarmonicOscillator_fast_time
    problem_class_first_order = HarmonicOscillator_fast_time_first_order
    sweeper_params["initial_guess"] = "collocation"
    model_sdc = sdc_class(
        problem__fast_time_params,
        collocation_params,
    ...
    model_sdc_first_order = sdc_class(
        problem__fast_time_params,
        collocation_params,
        sweeper_params,
        problem_class_first_order,
    )
    pos_fist_order, vel_first_order = model_sdc_first_order.sdc_iter(K)
    pos_solution, vel_solution = model_sdc.sdc_iter(K)
    ...
    Solution_set = [pos_solution, pos_first]
    plot_solution(time, Solution_set, Title, label_set)
