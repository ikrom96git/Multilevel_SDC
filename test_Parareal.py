...
def get_forceTerm(t, mu, kappa, F0, omega_force):
    ...
    position_force = Ap * get_sin(omega_force, t) + Bp * get_cos(omega_force, t)
    if c_omega == 0:
        velocity_force = Ap * get_dsin(omega_force, t) + (
            0.5 * F0 / (omega_force)
        ) * get_sin(omega_force, t)
    else:
        velocity_force = Ap * get_dsin(omega_force, t) + Bp * get_dcos(omega_force, t)
...
def const_withFriction(t, mu, kappa, F0=None, omega_force=None):
    ...
    A[0, 0], A[0, 1] = exp * get_cos(omega, t), exp * get_sin(omega, t)
