...
class Restriction(transfer_class):
    ...
    def fas_correction_operator(
        ...
        dt_coarse = coarse_zero_model.prob.dt
        F_fine = fine_model.build_f(
            X_fine[1:], V_fine[1:], dt_fine * fine_model.coll.nodes
        )
        F_coarse = coarse_zero_model.build_f(
            X_coarse, V_coarse, dt_coarse * coarse_zero_model.coll.nodes
        )
        RF_fine_vel = self.restrict(fine_model.coll.Q[1:, 1:] @ F_fine)
        RF_coarse_vel = coarse_zero_model.coll.Q[1:, 1:] @ F_coarse
        RF_fine_pos = self.restrict(fine_model.coll.QQ[1:, 1:] @ F_fine)
        RF_coarse_pos = coarse_zero_model.coll.QQ[1:, 1:] @ F_coarse
