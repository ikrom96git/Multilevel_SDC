...
class transfer_class(object):
    def __init__(self, problem_params, collocation_params, sweeper_params, problem_class):
        ...
        self.sdc_fine_level=sdc_class(problem_params, collocation_params_fine, sweeper_params, problem_class_fine)
        self.sdc_coarse_level=sdc_class(problem_params, collocation_params_coarse, sweeper_params, problem_class_coarse)
        self.Pcoll=self.get_transfer_matrix_Q(self.sdc_fine_level.coll.nodes, self.sdc_coarse_level.coll.nodes)
        self.Rcoll=self.get_transfer_matrix_Q(self.sdc_coarse_level.coll.nodes, self.sdc_fine_level.coll.nodes)
    
    @staticmethod
    def get_transfer_matrix_Q(f_nodes, c_nodes):
        approx=LagrangeApproximation(c_nodes)
        return approx.getIntegrationMatrix(f_nodes)
    ...
    def fas_correction(self, X_fine, V_fine):
        ...
        F_coarse=self.sdc_coarse_level.build_f(X_coarse, V_coarse, self.sdc_coarse_level.coll.nodes)
        RF_fine_vel=self.restrict(self.sdc_fine_level.coll.Q[1:,1:]@F_fine)
        RF_coarse_vel=self.sdc_coarse_level.coll.Q@F_coarse
        RF_fine_pos=self.restrict(self.sdc_fine_level.coll.QQ[1:,1:]@F_fine)
        RF_coarse_pos=self.sdc_coarse_level.coll.QQ@F_coarse
