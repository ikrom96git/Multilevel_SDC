...
class DuffingEquation(object):
    ...
    def odeint_rhs(self, y, t):
        x, v = y
        dydt = [v, -self.params.omega**2 * x + self.params.eps * (v**2) * x]
        return dydt
    ...
    def get_ntime_exact_solution(self, time):
        y0 = self.params.u0
        sol = odeint(self.odeint_rhs, y0, time)
        return sol[:, 0]
...
class DuffingEquation_first_order_problem(object):
    ...
    def build_f(self, X, V, T):
        ...
        const = (
            ...
            * (a**3)
            * (self.params.omega**2)
            * (np.sin(3 * self.params.omega * T) + np.sin(self.params.omega * T))
        )
    ...
    def get_exact_solution(self, t):
        ...
        x1 = (
            M1 * np.cos(z)
            + N1 * np.sin(z)
            - ((a**3) / 32) * (4 * z * np.cos(z) + np.sin(3 * z))
        )
        v1 = (
            -M1 * self.params.omega * np.sin(z)
            + N1 * self.params.omega * np.cos(z)
            - ((a**3) / 32)
            * (
                4 * self.params.omega * np.cos(z)
                - 4 * z * self.params.omega * np.sin(z)
                + 3 * self.params.omega * np.cos(z)
            )
        )
