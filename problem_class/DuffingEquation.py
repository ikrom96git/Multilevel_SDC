...
class DuffingEquation(object):
    ...
    def get_rhs(self, u, t):
        ...
        x_dot = u[1]
        v_dot = (
            -self.params.omega**2 * u[0] - self.params.eps * self.params.b * u[0] ** 3
        )
...
class DuffingEquation_zeros_order_problem(object):
    ...
    def get_exact_solution(self, t):
        ...
        v0 = -M0 * self.params.omega * np.sin(
            self.params.omega * t
        ) + N0 * self.params.omega * np.cos(self.params.omega * t)
...
class DuffingEquation_first_order_problem(object):
    ...
    def build_f(self, X, V, T):
        const = (
            ...
            * self.params.a**3
            * self.params.b
            * (np.cos(3 * self.params.omega * T) + 3 * np.cos(self.params.omega * T))
        )
    ...
    def get_exact_solution(self, t):
        ...
        v1 = (
            ...
            + M1
            * self.params.omega
            * (12 * np.sin(z) + 12 * z * np.cos(z) + 3 * np.sin(3 * z))
        )
